...
public class AdapterFotbal extends RecyclerView.Adapter<AdapterFotbal.ViewHolder> {

    List<MeciFotbal> matches;
    Context context;
    private int images[] = {R.drawable.viitorul,R.drawable.cfr_cluj,R.drawable.fcsb,R.drawable.dinamo,R.drawable.craiova,R.drawable.astra};
    final String[] options = {"Adauga pronostic"};

    public AdapterFotbal(List<MeciFotbal> matches, Context context) {
        this.matches = matches;
        this.context = context;
    }


    public AdapterFotbal.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_fotbal, parent, false);
        return new ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        MeciFotbal m = matches.get(position);
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        switch (which){
                                           ...
                                           //Toast.makeText(context,matches.get(position).getTeam1(),Toast.LENGTH_LONG).show();
                                            database.pronosticuriDao().insert(new Pronosticuri(matches.get(position).getId(),"fotbal",matches.get(position).getTeam1(),matches.get(position).getTeam2(),1));
                                            ...
                                            case 1:
                                                ...
                                                database2.pronosticuriDao().insert(new Pronosticuri(matches.get(position).getId(),"fotbal",matches.get(position).getTeam1(),matches.get(position).getTeam2(),2));
                                                for(String s :matches.get(position).getPlayersTeam1())
                                                {
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            team1Tv = (TextView)itemView.findViewById(R.id.team1FotbalTv);
            team2Tv = itemView.findViewById(R.id.team2FotbalTv);
            team1Img = itemView.findViewById(R.id.team1Fotbal);
            team2Img = itemView.findViewById(R.id.team2Fotbal);
            date = itemView.findViewById(R.id.dateFotbal);
            time = itemView.findViewById(R.id.timeFotbal);
            cardView =  itemView.findViewById(R.id.cardViewFotbal);
            scor1 = itemView.findViewById(R.id.scor1Fotbal);
            scor2 = itemView.findViewById(R.id.scor2Fotbal);
        }
    }
}